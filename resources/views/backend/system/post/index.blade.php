                                                ...
                                                <select name="post_category_id" class="form-select">
                                                    ...
                                                    @foreach($categories as $category)
                                                        <option value="{{ $category->id }}"
                                                                @if (request('post_category_id') == $category->id) selected @endif>
                                        ...
                                        <div class="dt-buttons btn-group flex-wrap">
                                            <div class="btn-group">
                                                ...
                                                @if(hasPermission('/' . strtolower($title) . '/*', 'put'))
                                                    <a class="btn add-new btn-primary text-white" href="{{ route('posts.create') }}">
                                            ...
                                            @if(hasPermission('/' . strtolower($title) . '/*', 'put') || hasPermission('/' . strtolower($title) . '/*', 'delete'))
                                                <div class="dropdown">
                                                    ...
                                                    <div class="dropdown-menu">
                                                        @if(hasPermission('/' . strtolower($title) . '/*', 'put'))
                                                            <a class="dropdown-item" href="{{ route('posts.edit', $post->id) }}"><i class="ti ti-pencil me-1"></i>{{ __('Edit') }}</a>
                                                        @endif
                                                        @if(hasPermission('/' . strtolower($title) . '/*', 'delete'))
                                                            <a href="#" class="dropdown-item delete-button" data-bs-toggle="modal" data-actionurl="{{ route('posts.destroy', $post->id) }}" data-bs-target="#deleteModal">

                        ...
                        <div class="dataTables_wrapper dt-bootstrap5 no-footer">
                                        ...
                                        <div class="dt-buttons btn-group flex-wrap">
                                            <div class="btn-group">
                                                ...
                                                @if(hasPermission('/'.strtolower($title).'/*','put'))
                                                    <a class="btn add-new btn-primary text-white" href="{{ route('sliders.create') }}">
                            ...
                            <table class="datatables table dataTable no-footer dtr-column" id="DataTables_Table_0" aria-describedby="DataTables_Table_0_info">
                                ...
                                @foreach ($thisDatas as $thisDatasKey => $thisDatasDatum)
                                    <tr>
                                        <td>{{ $thisDatasKey + 1 }}</td>
                                        <td>{{ $thisDatasDatum->title }}</td>
                                        <td>{{ $thisDatasDatum->position }}</td>
                                        <td><span class="badge @if ($thisDatasDatum->status) bg-label-primary @else bg-label-secondary @endif me-1">
                                            ...
                                            @if(hasPermission('/'.strtolower($title).'/*','put') || hasPermission('/'.strtolower($title).'/*','delete'))
                                                <div class="dropdown">
                                                    ...
                                                    <div class="dropdown-menu">
                                                        @if(hasPermission('/'.strtolower($title).'/*','put'))
                                                            <a class="dropdown-item" href="{{ route('sliders.edit', $thisDatasDatum->id) }}"><i class="ti ti-pencil me-1"></i>{{ __('Edit') }}</a>
                                                        @endif
                                                        @if(hasPermission('/'.strtolower($title).'/*','delete'))
                                                            <a href="#" class="dropdown-item delete-button" data-bs-toggle="modal" data-actionurl="{{ route('sliders.destroy', $thisDatasDatum->id) }}" data-bs-target="#deleteModal">
